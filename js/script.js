    ...
    history : [],
     ...
     callAPI: async function(url,query_string) {
      ...
      getCurrentWeather: function(city){
          ...
          Weather.properties.lattitude =  response.coord.lat;
          Weather.properties.longitude =  response.coord.lon; 
          Weather.getUVIndex();
          Weather.saveHistory($('#city').val());
          Weather.populateHistory();
        }) 
        .catch(error => {
          Weather.handleError(error.responseJSON.message);
        });
      },
      ...
      saveHistory : function(city){
        if(!this.history.includes(city) && city !="") {  
